...
if len(sys.argv) < 3:
    print(
        f"""FFmpeg CRT transform script / VileR 2021
...
output_file = (
    ...
    / "{}_{}{}".format(input_file.stem, config_file.stem, input_file.suffix)
)
...
result = run_command_get_params(
    f"ffprobe -hide_banner -loglevel quiet -select_streams v:0 -show_entries stream=width,height,nb_frames {input_file}",
    "width",
    "height",
    "nb_frames",
)
...
if None in (ix, iy, fc):
    print(
        f"Couldn't get media info for input file '{input_file}' (invalid image/video?)",
        file=sys.stderr,
    )
    sys.exit(1)
...
params.vsigma = (
    0.1 if params.v_px_blur == 0 else f"{params.v_px_blur}/100*{params.prescale_by}"
)
...
if params.monitor_color == "white":
    ...
    params.monocurves = (
        "curves=r='0/0 .77/0 1/.45':g='0/0 .77/1 1/1':b='0/0 .77/.17 1/.73',"
    )
    ...
    params.monocurves = (
        "curves=r='0/0 .5/.49 1/1':g='0/0 .5/.49 1/1':b='0/0 .5/.62 1/1',"
    )
    ...
    params.monocurves = (
        "curves=r='0/0 .46/.49 1/1':g='0/0 .46/.37 1/.94':b='0/0 .46/0 1/.29',"
    )
...
if params.monitor_color == "p7":
    params.monocurves_lat = (
        "curves=r='0/0 .6/.31 1/.75':g='0/0 .25/.16 .75/.83 1/.94':b='0/0 .5/.76 1/.97'"
    )
    params.monocurves_dec = (
        "curves=r='0/0 .5/.36 1/.86':g='0/0 .5/.52 1/.89':b='0/0 .5/.08 1/.13'"
    )
...
if params.scanlines_on:
    print("Scanlines")
    run_command(
        f"""ffmpeg -hide_banner -loglevel {loglevel} -stats -y -f lavfi
        -i nullsrc=s=1x100
        -vf "
                format=gray,
    ...
    run_command(
        f"""ffmpeg -hide_banner -loglevel {loglevel} -stats -y -loop 1 -framerate 1 -t {params.sl_count}
        -i TMPscanline.png
        -vf "
                format=gray16le,
...
if params.ovl_alpha > 0:
    run_command(
        f"""ffmpeg -hide_banner -loglevel {loglevel} -stats -y -i _{params.ovl_type}.png -vf "
    ...
    result = run_command_get_params(
        "ffprobe -hide_banner -loglevel quiet -show_entries stream=width,height TMPshadowmask1x.png",
        "width",
        "height",
    )
    ...
    run_command(
        f"""ffmpeg -hide_banner -loglevel {loglevel} -stats -y -loop 1 -i TMPshadowmask1x.png -vf "
  ...
  -frames:v 1 TMPshadowmask.png"""
    ...
    run_command(
        f"""ffmpeg -hide_banner -loglevel {loglevel} -stats -y -f lavfi -i "color=c=#00000000:s={params.px}x{params.py},format=rgba" -frames:v 1 TMPshadowmask.png"""
    )
    ...
    if params.texture_ovl == "paper":
        ...
        run_command(
            f"""ffmpeg -hide_banner -y -loglevel {loglevel} -stats -f lavfi -i "color=c=#808080:s={params.paperx}x{params.papery}"
        -filter_complex "
        ...
        print("Texture overlay")
        run_command(
            f"""ffmpeg -hide_banner -y -loglevel {loglevel} -stats -filter_complex "color=#808080:s={params.grainx}x{params.grainy},
...
if params.preprocess:
    ...
    run_command(
        f"""ffmpeg -hide_banner -loglevel {loglevel} -stats -y -i {input_file} -filter_complex "{params.vf_pre}" {tmp_outparams} TMPstep00.{tmp_ext}"""
    )
...
params.gridfilterfrag = (
    f"[scaled]; movie=TMPgrid.png[grid]; [scaled][grid]blend=all_mode={params.gridblendmode}"
    if params.flat_panel
    else ""
)
...
print("Step01")
run_command(
    f"""ffmpeg -hide_banner -loglevel {loglevel} -stats -y -i {params.scalesrc} -filter_complex "
...
params.crop_str = ""
crop_re = re.search(r"\scrop=(.+)$", crop_output, flags=re.MULTILINE)
if crop_re:
...
print("Output")
run_command(
    f"""ffmpeg -hide_banner -loglevel {loglevel} -stats -y -i TMPstep03.{tmp_ext} -filter_complex "
